...
class BaseAdmin(ModelAdmin):
    ...
    def save_model(self, request, instance, form, change):
        ...
        """       
        # Check tenant and setup
        related_tenant_fields = getattr(self, related_tenant_fields, [])
        if 'tenant' in related_tenant_fields: 
            if not getattr(instance, 'tenant', None):            
                instance.tenant = get_tenant_instance(request)
        if 'setup' in related_tenant_fields: 
            if not getattr(instance, 'setup', None):            
                instance.setup = APISetup.get_setup(tenant=tenant)

        # Proceed with logging
        queryset = save_logging(instance, request)
